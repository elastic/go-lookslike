...
func SingleResult(path paths.Path, result ValueResult) *Results {
	r := NewResults()
...
func (r *Results) Merge(other *Results) {
	for otherPath, valueResults := range other.Fields {
		for _, valueResult := range valueResults {
			r.Record(paths.MustParsePath(otherPath), valueResult)
		}
	}
}

// MergeUnderPrefix merges the given results at the path specified by the given prefix.
func (r *Results) MergeUnderPrefix(prefix paths.Path, other *Results) {
	if len(prefix) == 0 {
	...
	for otherPath, valueResults := range other.Fields {
		for _, valueResult := range valueResults {
			parsed := paths.MustParsePath(otherPath)
...
func (r *Results) DetailedErrors() *Results {
	errors := NewResults()
	r.EachResult(func(p paths.Path, vr ValueResult) bool {
		if !vr.Valid {
			errors.Record(p, vr)
		}
